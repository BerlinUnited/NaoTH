...
local function checkRecompileNeeded(file)
  ...
  print ("INFO: " .. file ..  " was *NOT* changed")
  return false
end

local function touchShadow(file, time)
  local shadowFile = getShadowFile(file)
  print("INFO: creating shadow file " .. shadowFile)
  io.output(shadowFile)
  io.write(time)
  io.close()  
end

local function protocCompile(files)
  ...
  local args = ""
  if(_OPTIONS["protoc-cpp"] ~= nil) then
    args = args .. "--cpp_out=\"" ..  _OPTIONS["protoc-cpp"] .. "\" " -- always mind the space at the end
  end
  if(_OPTIONS["protoc-java"] ~= nil) then
    args = args .. "--java_out=\"" ..  _OPTIONS["protoc-java"] .. "\" " -- always mind the space at the end
  end
  if(_OPTIONS["protoc-ipath"] ~= nil) then
    local splitted = split(_OPTIONS["protoc-ipath"], ":")
    for i=1,#splitted do
      args = args .. "--proto_path=\"" ..  splitted[i] .. "\" " -- always mind the space at the end
    end
  end

  args = args .. table.concat(files, " ")
...
newaction {
  ...
  execute = function ()
    ...
    local compile = false        
    for i = 1, #_ARGS do
      if(checkRecompileNeeded(_ARGS[i])) then
        compile = true
        break
      end
    end
