...
bool WebotsController::init(const std::string& modelPath, const std::string& teamName, unsigned int teamNumber, unsigned int playerNumber, const std::string& server, unsigned int port, bool sync)
{
...
void WebotsController::executeDebugCommand(const std::string &command, const std::map<std::string, std::string> &arguments, std::ostream &outstream)
{
  if(command == "beam")
  {
    Vector3d pose;
...
bool WebotsController::updateSensors(std::string& msg)
{
    ...
    while(sexp)
    {
      ...
      if (SexpParser::isVal(t))
      {
        ...
        else
        {
          ...
          {
            cerr << "[WebotsController] Perception unknow name: " << name << endl;
            ignore.insert(name);
          }
        }

        if (!ok) {
          cerr << "[WebotsController] Perception update failed: " << name << endl;
          return false;
        }
      }
      destroy_sexp(sexp);
...
int WebotsController::parseString(char* data, std::string& value)
{
  int c = 0;
  std::stringstream ss;
  while(data[c] != ' ' && data[c] != ')')
...
int WebotsController::parseInt(char* data, int& value)
{
  std::string tmp;
...
int WebotsController::parseImage(char* data)
{
  //(IMG (s 320 240) (
...
bool WebotsController::updateImage(const sexp_t* sexp)
{
  // size of image
  int size[2];
...
bool WebotsController::updateHingeJoint(const sexp_t* sexp)
{
  // get the name
  std::string name;
...
bool WebotsController::updateGyro(const sexp_t* sexp)
{
  // get the name
  std::string name;
  if (!SexpParser::parseGivenValue(sexp, "n", name))
...
bool WebotsController::updateAccelerometer(const sexp_t* sexp)
{
  // get the name
  std::string name;
  if (!SexpParser::parseGivenValue(sexp, "n", name))
...
bool WebotsController::updateBattery(const sexp_t* sexp)
{
...
bool WebotsController::updateGPS(const sexp_t* sexp)
{
  std::string name;
  if (!SexpParser::parseGivenValue(sexp, "n", name))
  {
...
bool WebotsController::updateGameInfo(const sexp_t* sexp)
{
  bool ok = true;
  string name;
  while (sexp)
    ...
    if (SexpParser::parseValue(t, name))
    {
      ...
      } else if ("pm" == name) // play mode
      {
        ...
        if (state == GameData::unknown_game_state)
        {
          ...
          WebotsGameInfo::PlayMode playMode = WebotsGameInfo::getPlayModeByName(pm);
          if(playMode != WebotsGameInfo::numOfPlayMode) {
            state = WebotsGameInfo::covertToGameState(playMode);
          }
        }
        else
        {
...
bool WebotsController::parseTeamInfo(const sexp_t* team, std::vector<naoth::GameData::RobotInfo> &players)
{
...
bool WebotsController::updateFSR(const sexp_t* sexp)
{
  // get the name
  std::string name;
  if (!SexpParser::parseGivenValue(sexp, "n", name))
...
Vector3d WebotsController::decomposeForce(double f, double fx, double fy, const Vector3d& c0, const Vector3d& c1, const Vector3d& c2)
{
  Matrix3x3<double> A(Vector3d(1, c0.x, c0.y), Vector3d(1, c1.x, c1.y), Vector3d(1, c2.x, c2.y));
  return A.invert() * Vector3d(f, fx, fy);
}

void WebotsController::calFSRForce(
...
bool WebotsController::updateSee(VirtualVision& virtualVision, const sexp_t* sexp)
{
  bool ok = true;
...
bool WebotsController::parsePoint3D(const sexp_t* sexp, Vector3d& result) const
{
...
void WebotsController::updateInertialSensor()
{
...
void WebotsController::jointControl()
{
...
bool WebotsController::hear(const sexp_t* sexp)
{
...
void WebotsController::beam(const Vector3<double>& p)
{
    theActData << "(beam "<<p.x<<" "<<p.y<<" "<<p.z<<")";
}

void WebotsController::autoBeam()
{
...
bool WebotsController::updateIMU(const sexp_t* sexp)
{
  // get the name
  std::string name;
  if (!SexpParser::parseGivenValue(sexp, "n", name))
