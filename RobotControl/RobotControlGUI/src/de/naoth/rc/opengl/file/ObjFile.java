...
public class ObjFile implements GLFile {
    ...
    public ObjFile(String objFilename) {
    ...
    private void processFace(String line) {    	
        ...
        for (int i = 1; i < cornerData.length; i++) {
            ...
            if(glNormalData.get(faceIndex) == null) {
            	glTextureData.set(faceIndex, texture);
            	glNormalData.set(faceIndex, normal);
            	
            	glIndexData.add(faceIndex);
            }
            else if (glNormalData.get(faceIndex) == normal && glTextureData.get(faceIndex) == texture) {
            	glIndexData.add(faceIndex);
            }
            else {
    ...
    public GLData buildGLData() {
        return new GLData(glVertexData, glNormalData, glTextureData, glIndexData, loadImage());
    }
    
    protected BufferedImage loadImage() {
    	BufferedImage image = null;
        try {
            image = ImageIO.read(new File(System.getProperty("user.dir").replaceAll("\\\\", "/") + "/src/de/naoth/rc/opengl/res/" + objName));
        } catch (IOException io) {
            System.out.println("Unable to find the texture file: " + objName);
            try {
            image = ImageIO.read(new File(System.getProperty("user.dir").replaceAll("\\\\", "/") + "/src/de/naoth/rc/opengl/res/replacement.png"));
            } catch (IOException e) {
