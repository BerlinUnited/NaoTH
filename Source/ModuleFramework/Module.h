...
public:
  ...
  const list<Representation*>& getRecycledRepresentations()
  {
    return recycled;
  }
...
protected:
  ...
  static RepresentationMap* static_recycling_registry;
  ...
  template<class TYPE_WHAT>
  class StaticRecyclesRegistrator
  {
  private:
    TypedRegistrationInterface<TYPE_WHAT>* data;

  public:
    StaticRecyclesRegistrator(){}

    StaticRecyclesRegistrator(const std::string& name)
    {
      // TODO: check the type
      if(static_recycling_registry->find(name) == static_recycling_registry->end())
      {
        (*static_recycling_registry)[name] = new TypedRegistrationInterface<TYPE_WHAT>(name);
      }
      // HACK
      data = (TypedRegistrationInterface<TYPE_WHAT>*)(*static_recycling_registry)[name];
    }
...
template<class T>
RepresentationMap* StaticRegistry<T>::static_recycling_registry = new RepresentationMap();
...
#define RECYCLES(representationName) \
  private: \
  class representationName##StaticRecyclesRegistrator : public StaticRecyclesRegistrator<representationName>\
  { \
  public: representationName##StaticRecyclesRegistrator() : StaticRecyclesRegistrator<representationName>(#representationName){} \
  }the##representationName; \
  protected: \
  inline const representationName& get##representationName() const \
  { \
...
#define PROVIDE(representationName) \
  ...
  { \
  public: representationName##StaticProvidingRegistrator() : StaticProvidingRegistrator<representationName>(#representationName){} \
  } the##representationName; \
  protected: \
