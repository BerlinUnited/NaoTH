...
if [ -d "$DEPLOY_DIR/home/nao/naoqi/Config" ]; then
  rm -rf /home/nao/Config/*
  su nao -c "cp -r $DEPLOY_DIR/home/nao/naoqi/Config/* /home/nao/Config/"
fi
...
if [ ! -f "/opt/aldebaran/bin/lola" ] && [ ! -f "/usr/bin/lola" ]; then
  ...
  if [ -f "$DEPLOY_DIR/home/nao/bin/libnaosmal.so" ]; then
    ...
    su nao -c "cp $DEPLOY_DIR/home/nao/bin/libnaosmal.so /home/nao/bin/libnaosmal.so"
    /etc/init.d/naoqi start
  fi
else
  # copy binaries and start naoqi/naoth again
  if [ -f "$DEPLOY_DIR/home/nao/bin/lola_adaptor" ]; then
    echo "[deploy] replace lola_adaptor"
    lola_adaptor stop
    rm -f /home/nao/bin/lola_adaptor
    su nao -c "cp $DEPLOY_DIR/home/nao/bin/lola_adaptor /home/nao/bin/lola_adaptor"
    chmod 755 /home/nao/bin/lola_adaptor
  fi

fi

if [ -f "$DEPLOY_DIR/home/nao/bin/naoth" ]; then
  echo "[deploy] replace naoth"
  rm -f /home/nao/bin/naoth
  su nao -c "cp $DEPLOY_DIR/home/nao/bin/naoth /home/nao/bin/naoth"
...
if [ -f "/var/log/messages" ]; then
  ...
  journalctl -e --user-unit=naoth -n 1000 > ./braindump.txt
fi
