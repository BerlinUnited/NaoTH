...
class Actions:
    ...
    def get_actions(self):
        return [self.kick_short, self.sidekick_left, self.sidekick_right, self.turn_left_45, self.turn_right_45]

    def kick_short(self, state):
        state.ball_position = self.short.predict(state.ball_position, self.add_noise) # noise included

    def sidekick_left(self, state):
        state.ball_position = self.left.predict(state.ball_position, self.add_noise) # noise included

    def sidekick_right(self, state):
        state.ball_position = self.right.predict(state.ball_position, self.add_noise) # noise included
