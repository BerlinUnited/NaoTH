...
def reward_state_termination_wrapper(state, action):
    ...
    derive_new_state(new_state, action) # set state to the new state which get derive from the world model
...
def calculate_mean(old_mean, num_of_iterations, new_value):
    new_mean = old_mean * (1. - 1/float(num_of_iterations)) + new_value * (1/float(num_of_iterations))
    return new_mean
...
def run(save_model=True, load_model=False, save_mean=False):
    ...
    print "\nrunning...\n"
    print "run: " + str(run) + "; length: " + str(run_length)
    print "   -------   "
    ...
    while True:
        ...
        for i in range(1, 201):
            ...
            initial_state.position.x = np.random.randint(-4500, 0)  # get random position in own field
            ...
            overall_reward, episode_length = SimAgent.run_sarsa(initial_state,
                                                 lambda state, action: reward_state_termination_wrapper(
                                                     state, action))
            mean = calculate_mean(mean, i % 50 + 1  , overall_reward)
            if (i % 50) == 0:
                print "Mean reward on run " + str(i-50) + " - " + str(i)
                print mean
                print "Max Norm:"
                print np.linalg.norm(SimAgent.theta, np.inf)
                print "   -------   "
