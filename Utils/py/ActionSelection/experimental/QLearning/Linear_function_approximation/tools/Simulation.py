...
def simulate_consequences(action, categorized_ball_positions, state, num_particles):
    ...
    opp_goal_back_left = Vector2(field.opponent_goalpost_left.x + field.goal_depth, field.opponent_goalpost_left.y)
    opp_goal_back_right = Vector2(field.opponent_goalpost_right.x + field.goal_depth, field.opponent_goalpost_right.y)
    ...
    obstacle_line = LineSegment(state.pose * Vector2(400, 200), state.pose *.Vector2(400, -200))
    ...
    categorized_ball_positions.expected_ball_pos_mean = Vector2(np.mean(mean_test_list_x), np.mean(mean_test_list_y))
    categorized_ball_positions.expected_ball_pos_median = Vector2(np.median(mean_test_list_x), np.median(mean_test_list_y))
