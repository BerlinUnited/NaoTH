...
from .naoth_helpers import write_naoth_header_file
...
class NNCG:
    ...
    def keras_compile(self, imdb, model, code_path, identifier=None, image_mean=99, arch="general",
                      quantization=False, weights_method='direct'):
        """
        ...
        :param quantization: Convert applicable layers to unit8?
        :return: None.
        """
        path = code_path
        ...
        # TODO this only works for dense layers as output
        # this is only used for creating the naoth header file
        output_shape = model.layers[-1].output.shape[1:].as_list()[0]

        STEPS = 4
        print_progress_bar(0, STEPS, prefix='Adding CNN nodes to graph')

        self.root_node = Edge('root', CHeaderNode(identifier, input_shape, weights_method, class_name), None, 'forward')
        ...
        CFooterNode(exe_return_filename, weights_method, cur_node, class_name)
        ...
        # patch the generated cpp file for last layer output name
        # TODO
        fin = open(code_path, "rt")
        data = fin.read()
        # replace all occurrences of the required string
        data = data.replace(str(CFooterNode.instance().in_var), 'scores')
        fin.close()
        fin = open(code_path, "wt")
        fin.write(data)
        fin.close()

        write_naoth_header_file(input_shape, output_shape, class_name, output_folder)
