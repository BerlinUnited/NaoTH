...
'''
    ...
    def test(self, im, exit_on_err):
        ...
        # We have to allow a small error due to rounding errors
        if exit_on_err and not np.allclose(res, c_res, atol=0.000001 * np.max(res)):
            print("Check of variable {} for layer {}.".format(self.in_var, self.layer_name))
            idx = np.unravel_index(np.argmax(res - c_res), res.shape)
