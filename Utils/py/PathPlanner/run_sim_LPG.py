...
while loop_bool:
    ...
    if robot_pos[0] == orig_target[0] and robot_pos[1] == orig_target[1]:
        ...
        if exp_count > 1 and len(argv) > 1:
            file.write(str(actual_path) + ", " + str(obstacles) + ", " + str(target) + ", " + str(robot_pos))
            file.write("\n")
        ...
        while len(obstacles) < 9:
            ...
            if obst_x <= (obst_r + LPG.robot_radius) and obst_x >= -1*(obst_r + LPG.robot_radius) and obst_y <= (obst_r + LPG.robot_radius) and obst_y >= -1*(obst_r + LPG.robot_radius):
                continue
