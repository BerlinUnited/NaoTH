# Code from https://github.com/KonradIT/gopro-py-api
...
class GoPro:
    ...
    def gpControlSet(self, param, value):
      ...
      try:
          return urllib.request.urlopen('http://' + self.ip_addr + '/gp/gpControl/setting/' + param + '/' + value,
    ...
    def sendCamera(self, param, value=""):
      ...
      try:
        urllib.request.urlopen('http://' + self.ip_addr + '/camera/' + param + '?t=' + self.getPassword() + value_notempty,
                               timeout=5).read()
      except (HTTPError, URLError) as error:
        print("Error code:" + str(error.code) + "\nMake sure the connection to the WiFi camera is still active.")
      except timeout:
    ...
    def sendBacpac(self, param, value):
      ...
      if value:
        value_notempty = str('&p=%' + value)
      try:
        urllib.request.urlopen('http://' + self.ip_addr + '/bacpac/' + param + '?t=' + self.getPassword() + value_notempty,
                               timeout=5).read()
      except (HTTPError, URLError) as error:
        print("Error code:" + str(error.code) + "\nMake sure the connection to the WiFi camera is still active.")
      except timeout:
    ...
    def delete(self, option):
        if self.whichCam() == "gpcontrol":
            if isinstance(option,
                          int):  # This allows you to delete x number of files backwards. Will delete a timelapse/burst entirely as its interpreted as a single file.
    ...
    def getMediaInfoFront(self, option):
      folder = ""
      file = ""
      size = ""
      try:
    ...
    def syncTime(self):
      ...
      datestr = str("%" + str(datestr_year) + "%" + str(datestr_month) + "%" + str(datestr_day) + "%" + str(datestr_hour) + "%" + str(
        datestr_min) + "%" + str(datestr_sec))
      if self.whichCam() == "gpcontrol":
        ...
        if path == "":
          if GPR == True:
            if path.endswith("JPG"):
              urllib.request.urlretrieve(self.getMedia().replace("JPG", "GPR"),
                                         self.getMediaInfo("folder") + "-" + self.getMediaInfo("file"))
            else:
          ...
          if GPR == True:
            if path.endswith("JPG"):
              if custom_filename == "":
                urllib.request.urlretrieve(self.getMedia().replace("JPG", "GPR"),
                                           self.getMediaInfo("folder") + "-" + self.getMediaInfo("file").replace("JPG", "GPR"))
              else:
    ...
    def getVideoInfo(self, option="", folder="", file=""):
      if option == "":
        if folder == "" and file == "":
          if self.getMediaInfo("file").endswith("MP4"):
            return urllib.request.urlopen(
              'http://' + self.ip_addr + ':8080/gp/gpMediaMetadata?p=' + self.getMediaInfo("folder") + "/" + self.getMediaInfo(
                "file") + '&t=videoinfo').read().decode('utf-8')
      else:
        data = ""
        if folder == "" and file == "":
        ...
        if folder == "":
          if not file == "":
            if file.endswith("MP4"):
              data = urllib.request.urlopen('http://' + self.ip_addr + ':8080/gp/gpMediaMetadata?p=' + self.getMediaInfo(
      ...
      if option == "":
        ...
        if folder == "":
          if not file == "":
            if file.endswith("JPG"):
              data = urllib.request.urlopen('http://' + self.ip_addr + ':8080/gp/gpMediaMetadata?p=' + self.getMediaInfo(
                "folder") + "/" + file + '&t=v4info').read().decode('utf-8')
        if not file == "" and not folder == "" and file.endswith("JPG"):
          data = urllib.request.urlopen(
            'http://' + self.ip_addr + ':8080/gp/gpMediaMetadata?p=' + folder + "/" + file + '&t=v4info').read().decode('utf-8')
        jsondata = json.loads(data)
    ...
    def getClip(self, file, resolution, frame_rate, start_ms, stop_ms):
        out = self.gpControlCommand(
          "transcode/request?source=DCIM/" + file + "&res=" + resolution + "&fps_divisor=" + frame_rate + "&in_ms=" + start_ms + "&out_ms=" + stop_ms)
    ...
    def overview(self):
        if self.whichCam() == "gpcontrol":
            ...
            print("current video resolution: " + "" + self.parse_value("video_res", self.getStatus(constants.Status.Settings,
            ...
            print("space left in sd card: " + "" + self.parse_value("rem_space", self.getStatus(constants.Status.Status,
            ...
            print("current video resolution: " + self.parse_value(constants.Hero3Status.VideoRes,
                                                                  self.getStatus(constants.Hero3Status.VideoRes)))
            print(
              "current photo resolution: " + self.parse_value(constants.Hero3Status.PicRes, self.getStatus(constants.Hero3Status.PicRes)))
