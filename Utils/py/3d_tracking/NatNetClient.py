...
class NatNetClient:
    def __init__( self ):
        ...
        self.serverIPAddress = "10.0.4.254"
        ...
        self.__natNetStreamVersion = (2,3,0,0)
    ...
    def __unpackRigidBody( self, data ):
        ...
        bodyPos = Vector3.unpack( data[offset:offset+12] )
        offset += 12
        trace( "\tPosition:", bodyPos[0],",", bodyPos[1],",", bodyPos[2] )
        ...
        if self.rigidBodyListener is not None:
            self.rigidBodyListener( id, bodyPos, rot, markers)
    ...
    def __unpackMocapData( self, data ):
        ...
        if( ( self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] > 0 ) or self.__natNetStreamVersion[0] > 2 ):
            #print(offset, len(data), data[offset], data[offset+1], data[offset+2], data[offset+3])
        ...
        if self.newFrameListener is not None:
            self.newFrameListener( frameNumber, markerSetCount, unlabeledMarkers, rigidBodyCount, skeletonCount,
    ...
    def __dataThreadFunction( self, socket, stop_event ):
    ...
    def run( self ):
        ...
        dataThread = Thread( target = self.__dataThreadFunction, args = (self.dataSocket, self.t1_stop,))

        self.t2_stop = threading.Event()
        # Create a separate thread for receiving command packets
        commandThread = Thread( target = self.__commandThreadFunction, args = (self.commandSocket, self.t2_stop,))
