...
class NatNetClient:
    ...
    def __unpackMocapData(self, data):
        ...
        if self.newFrameListener is not None:
            self.newFrameListener(frameNumber, markerSetCount, unlabeledMarkers, rigidBodyCount,
                                  skeletonCount,
                                  labeledMarkers, latency, timecode, timecodeSub, timestamp,
    ...
    def run(self):
        ...
        dataThread = Thread(target=self.__dataThreadFunction,
                            args=(self.dataSocket, self.t1_stop,))

        self.t2_stop = threading.Event()
        # Create a separate thread for receiving command packets
        commandThread = Thread(target=self.__commandThreadFunction,
                               args=(self.commandSocket, self.t2_stop,))
