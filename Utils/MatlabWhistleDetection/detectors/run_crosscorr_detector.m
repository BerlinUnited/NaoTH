...
% ref_set_name = 'bhuman_old';
% ref_set_name = 'bhuman';
% ref_set_name = 'go17_old';
% ref_set_name = 'go17';
% ref_set_name = 'gen_from_go17_recordings';
% ref_set_name = 'kln17';
% ref_set_name = 'rc17';
% ref_set_name = 'go18_old';
% ref_set_name = 'go18';
% ref_set_name = 'gen_from_go18_recordings';

ref_names = fieldnames(reference_database);
for t = 1:length(ref_names)
    ref_set_name = ref_names(t);
    ref = reference_database.(char(ref_set_name));
    ...
    for t = 1:length(recordings_names)
        ...
        for c = 1:length(recording)
            ...
            if ~isempty(recording(c).annotations)
                ...
                total_whistle_event_count = sum(recording(c).annotations(:,1));
        ...
        if create_comparison_plots && has_any_annotation
            if exist(strcat(figure_export_path, recording_name, "/ref_", ref_set_name, 'dir')) ~= 7
                mkdir(char(strcat(figure_export_path, recording_name, "/ref_", ref_set_name)))
            end
