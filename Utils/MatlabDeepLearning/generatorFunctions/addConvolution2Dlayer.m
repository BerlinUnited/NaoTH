function [out_dim_x, out_dim_y] = addConvolution2Dlayer(HeaderFile,BodyFile, step,layer,in_dim_x,in_dim_y)
...
%   Detailed explanation goes here    
    ...
    for x = -padding_x + 1 : stride_x : in_dim_x + padding_x - filter_dim_x + 1
        ...
        for y = -padding_y + 1 : stride_y : in_dim_y + padding_y - filter_dim_y + 1
            ...
            fprintf(BodyFile, 'out_step%d[%d][%d] = ', step, x_out, y_out);
            i = 0;
            % 1 > x,y cords > in_dim
            % mat to c cords :  -1
            for f_x = 1:filter_dim_x
                for f_y = 1:filter_dim_y
                   if (x+f_x-1 < 1 || x+f_x-1 > in_dim_x) || (y+f_y-1 < 1 || y+f_y-1 > in_dim_y) 
                       ...
                       fprintf(BodyFile, '%.8f * out_step%d[%2d][%2d]', abs(weights(f_x,f_y)), step-1, x+f_x-1-1, y+f_y-1-1);
