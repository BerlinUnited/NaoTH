...
function [value,isterminal,direction] = events_flying(t,y,l0,touchdown_angle)
    ...
    value = [y(5); y(5) + l0 * cos(touchdown_angle(1))]; % [fallen down; z of touchdown point = 0]
    isterminal = [1; 1];
    direction  = [0;-1];
end
...
function [value,isterminal,direction] = events_doubleSupport(t,y,l0,left_tdp,right_tdp)
    ...
    left_leg  = [y(1); y(3); y(5)] - left_tdp;
    right_leg = [y(1); y(3); y(5)] - right_tdp;  
    value = [y(5); l0 - norm(left_leg); l0 - norm(right_leg)]; % [fallen down; left liftoff; right liftoff)
...
function [value,isterminal,direction] = events_singleSupport(t,y,l0,tdp,tda)
    leg = [y(1); y(3); y(5)] - tdp; 
    value = [y(5); l0 - norm(leg); y(5) + l0 * cos(tda(1))]; % [fallen down; liftoff; touchdown]
    isterminal = [1;  1;  1]; % stop the integration because dynamics have to be switchted,
    direction  = [0; -1; -1]; % negative direction
end
