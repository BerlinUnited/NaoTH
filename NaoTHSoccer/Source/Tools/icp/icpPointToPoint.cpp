...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r10,r11,t0,t1) reduction(+:mum0,mum1, mut0,mut1) // schedule (dynamic,2)
    for (i=0; i<nact; i++) {
      ...
      Vector2d p;
      p.x = (float)(r00*T[idx*2+0] + r01*T[idx*2+1] + t0);
      p.y = (float)(r10*T[idx*2+0] + r11*T[idx*2+1] + t1);

      // search nearest neighbor
      //M_tree->n_nearest(query,1,result);
      LinesTable::NamedPoint line_point_global = fieldLinesTable.get_closest_point(p, LinesTable::all_lines);

      // set model point
      p_m.val[i][0] = line_point_global.position.x; mum0 += p_m.val[i][0];
      p_m.val[i][1] = line_point_global.position.y; mum1 += p_m.val[i][1];

      // set template point
      p_t.val[i][0] = p.x; mut0 += p_t.val[i][0];
      p_t.val[i][1] = p.y; mut1 += p_t.val[i][1];
    }
    mu_m.val[0][0] = mum0;
    mu_m.val[0][1] = mum1;
