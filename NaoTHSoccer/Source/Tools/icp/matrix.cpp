...
Matrix::Matrix (const int32_t m_,const int32_t n_,const double* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
void Matrix::getData(double* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(double s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(double s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const double &angle) {
  double s = sin(angle);
  double c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const double &angle) {
  double s = sin(angle);
  double c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const double &angle) {
  double s = sin(angle);
  double c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
bool Matrix::lu(int32_t *idx, double &d, double eps) {
  ...
  double* vv = (double*)malloc(n*sizeof(double)); // vv stores the implicit scaling of each row.
  d = 1.0;
