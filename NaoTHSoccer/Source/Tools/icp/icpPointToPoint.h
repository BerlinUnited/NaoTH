...
public:
  ...
  IcpPointToPoint (const LinesTable& fieldLinesTable, double *M,const int32_t M_num,const int32_t dim) : Icp(M,M_num,dim),fieldLinesTable(fieldLinesTable) {}
  virtual ~IcpPointToPoint () {}

private:
  const LinesTable& fieldLinesTable;
