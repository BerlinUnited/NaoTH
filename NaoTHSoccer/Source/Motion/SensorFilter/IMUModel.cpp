...
IMUModel::IMUModel(){
    ...
    ukf_acc_global.R << 5.074939351879890342e-04, -1.561730283237946278e-05,  1.012849085655689321e-04,
    ...
    ukf_rot.Q = Eigen::Matrix<double,6,6>::Identity(); // covariance matrix of process noise
...
void IMUModel::execute(){
    ...
    if(fabs(acceleration.norm() - 9.81) < 0.5){ // use acceleration
    ...
    ukf_acc_global.predict(dt);
    ukf_acc_global.update(z_acc);
