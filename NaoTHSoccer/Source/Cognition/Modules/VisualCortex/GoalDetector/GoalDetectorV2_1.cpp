...
/**
* @file GoalDetectorV2_1.cpp
*
* Implementation of class GoalDetectorV2_1
*
*/

#include "GoalDetectorV2_1.h"
...
GoalDetectorV2_1::GoalDetectorV2_1()
: 
  cameraID(CameraInfo::Bottom)
{
  ...
  DEBUG_REQUEST_REGISTER("Vision:GoalDetectorV2_1:markPostCenterBackProjected","..", false);
  DEBUG_REQUEST_REGISTER("Vision:GoalDetectorV2_1:markPostBackProjected","..", false);

  DEBUG_REQUEST_REGISTER("Vision:GoalDetectorV2_1:showColorByHistogramBottom","..", false);
  DEBUG_REQUEST_REGISTER("Vision:GoalDetectorV2_1:showColorByHistogramTop","..", false);
...
void GoalDetectorV2_1::execute(CameraInfo::CameraID id)
{
  cameraID = id;
...
void GoalDetectorV2_1::clusterEdgelFeatures()
{
...
void GoalDetectorV2_1::calcuateGoalPosts()
{
  bool markValidPosts = false; 
  bool markInvalidPosts = false; 
  DEBUG_REQUEST("Vision:GoalDetectorV2_1:markValidPosts",  
    markValidPosts = true;   
  );
  DEBUG_REQUEST("Vision:GoalDetectorV2_1:markInvalidPosts",  
    ...
    if(cluster.size() >= params.minGoodPoints) 
    {
      ...
      if(projectionOk) 
      {
        if(post.position.abs() > fieldDiagonal)
        {
          ...
          DEBUG_REQUEST("Vision:GoalDetectorV2_1:markPosts",  
            if(markInvalidPosts)
            {
              Vector2i begin;
              Vector2i end;
              ...
              ColorClasses::Color col = ColorClasses::black;
        ...
        else
        {
          ...
          DEBUG_REQUEST("Vision:GoalDetectorV2_1:markPostCenterBackProjected",  
          ...
          DEBUG_REQUEST("Vision:GoalDetectorV2_1:markPostBackProjected",  
            Vector2i begin;
            Vector2i end;
            begin = base;
            end = top;
          ...
          DEBUG_REQUEST("Vision:GoalDetectorV2_1:markPostCenterLine",  
...
Vector2i GoalDetectorV2_1::scanForEndPoint(const Vector2i& start, const Vector2d& direction) const
{
  Vector2i pos(start);
  Pixel pixel;
  ...
  while(footPointScanner.getNextWithCheck(pos))
  {
    ...
    DEBUG_REQUEST("Vision:GoalDetectorV2_1:markPostScans", 
      if(filter.ready()) {
...
void GoalDetectorV2_1::debugStuff(CameraInfo::CameraID camID)
{
  cameraID = camID;
  DEBUG_REQUEST("Vision:GoalDetectorV2_1:showColorByHistogramBottom",
  ...
  DEBUG_REQUEST("Vision:GoalDetectorV2_1:showColorByHistogramTop",
    for(unsigned int x = 0; x < getImage().width(); x+=2)
    {
      for(unsigned int y = 0; y < getImage().height(); y+=2)
      {
...
Vector2d GoalDetectorV2_1::getBackProjectedTopPoint(const GoalPercept::GoalPost& post)
{
...
Vector2d GoalDetectorV2_1::getBackProjectedBasePoint(const GoalPercept::GoalPost& post)
{
  Vector2d p1 = post.position;
...
int GoalDetectorV2_1::getBackProjectedTopBarWidth(const GoalPercept::GoalPost& post)
{
  Vector2d p1 = post.position;
...
int GoalDetectorV2_1::getBackProjectedBaseBarWidth(const GoalPercept::GoalPost& post)
{
  Vector2d p1 = post.position;
  Vector2d p1_ = post.position;
  p1_.rotateRight().normalize();
...
int GoalDetectorV2_1::getBackProjectedPostHeight(const GoalPercept::GoalPost& post)
{
