...
{
  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:edgel_pairs", "mark the edgels on the image", false);
  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:draw_neighbors", "mark the edgels on the image", false);
  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:draw_neighbors_field", "mark the edgels on the image", false);

  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:edgel_cluster", "mark the edgels on the image", false);
  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:draw_lines", "draw the esimated lines in the image", false);

  DEBUG_REQUEST_REGISTER("Vision:CompassProvider:draw_compas", "draw compas direcion based on the edgel directions", false);

  //DEBUG_REQUEST_REGISTER("Vision:CompassProvider:draw_line_graph", "", false);
...
void CompassProvider::execute(CameraInfo::CameraID id)
{
  cameraID = id;
  ...
  //    getCompassPercept().edgels.push_back(edgel);
  //      
  //    DEBUG_REQUEST("Vision:CompassProvider:draw_line_graph",
  ...
  //      getCompassPercept().edgelsInImageTop.push_back(getScanLineEdgelPercept().pairs[i]);
  //    } else {
  //      getCompassPercept().edgelsInImage.push_back(getScanLineEdgelPercept().pairs[i]);
  //    }
  //  }
  //}
  ...
  //DEBUG_REQUEST("Vision:CompassProvider:edgel_pairs",
  ...
  //DEBUG_REQUEST("Vision:CompassProvider:draw_neighbors",
  //  CANVAS(((cameraID == CameraInfo::Top) ? "ImageTop" : "ImageBottom"));
  //for (size_t i = 0; i < edgelNeighbors.size(); i++)
  //{
  ...
  //DEBUG_REQUEST("Vision:CompassProvider:draw_neighbors_field",
  //  FIELD_DRAWING_CONTEXT;
  //for (size_t i = 0; i < edgelNeighbors.size(); i++)
  //{
  //  const ScanLineEdgelPercept::EdgelPair& edgel = getScanLineEdgelPercept().pairs[i];
...
//double CompassProvider::edgelSim(const EdgelT<double>& e1, const EdgelT<double>& e2)
//{
//  double s = 0.0;
...
//void CompassProvider::calculatePairsAndNeigbors(
