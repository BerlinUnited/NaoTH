...
void BodyContourProvider::add(const Pose3D& origin, const std::vector<Vector3d>& c, double sign, BodyContour::BodyPartID id)
{
  ...
  cm.rotateY(NaoInfo::robotDimensions.cameraTransform[cameraID].rotationY + yOffsetCamera);
    ...
    if (q1_ok && q2_ok)
    {
      ...
      if(lineCutsImage)
      {
        if(q2.y < 0 && b.y == 0) {
          ...
          updateBodyContour(b, Vector2i(x2, 0), id);
        }

        if(q1.y < 0 && a.y == 0) {
          int x1 = Math::clamp(q1.x, frameUpperLeft.x, frameLowerRight.x);
          updateBodyContour(Vector2i(x1, 0), a, id);
        }
      }
      else
      {
        if(q1.y < 0 || q2.y < 0) {
          ...
          int x2 = Math::clamp(q2.x, frameUpperLeft.x, frameLowerRight.x);
          if(x1 != x2) {
            updateBodyContour(Vector2i(x1, 0), Vector2i(x2, 0), id);
          }
        }
      }
    }// end if projected
