...
void FieldColorClassifierFull::execute()
{
  ...
  for(unsigned int i = 0; i < COLOR_CHANNEL_VALUE_COUNT; i++)
  {
    ...
    weightedHistCr[i] = wCr * (double) getHistograms().histogramV.rawData[i];
    weightedHistTopCr[i] = wCr * (double) getHistogramsTop().histogramV.rawData[i];
  ...
  idxTop = maxWeightedIndexTopCr;
  while (idxTop < COLOR_CHANNEL_VALUE_COUNT && meanRegionEndIndexTopCr == 0)
  {
  ...
  idxTop = maxWeightedIndexTopY;
  while (idx < COLOR_CHANNEL_VALUE_COUNT && meanRegionEndIndexTopY == 0)
  {
  ...
  idxTop = maxWeightedIndexTopCb;
  while (idxTop < COLOR_CHANNEL_VALUE_COUNT && meanRegionEndIndexTopCb == 0)
  {
