...
int StableBallDetector::ckecknearBall(const Vector2i& start, Vector2d& center, double& radius)
{
  ...
  */
  ...
  calculateCircle(endPoints, center, radius);
...
bool StableBallDetector::calculateCircle( const std::vector<Vector2i>& ballPoints, Vector2d& center, double& radius )
{
  if (ballPoints.size() < 3) {
    return false;
  }
