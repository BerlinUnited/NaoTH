...
void SimpleFieldColorClassifier::execute()
{
  ...
  for(int i = 0; i < halfChannelWidth; i++)
  {
    ...
    double weightedCr = wCr * (double) getHistogram().colorChannelHistogramField[2][i];
  ...
  */
  ...
  for(unsigned int i = 0; i < COLOR_CHANNEL_VALUE_COUNT; i++)
  {
    ...
    double weightedCb = wCb * (double) getHistogram().weightedHistCb[i];
    ...
    meanFieldY += getHistogram().weightedHistY[i]*i;
    numberOfFieldY += getHistogram().weightedHistY[i];
  }//end for
