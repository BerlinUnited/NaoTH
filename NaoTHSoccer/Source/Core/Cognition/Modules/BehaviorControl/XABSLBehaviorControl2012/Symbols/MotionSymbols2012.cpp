/**
* @file MotionSymbols2012.cpp
*
* @author <a href="mailto:mellmann@informatik.hu-berlin.de">Heinrich Mellmann</a>
* Implementation of class MotionSymbols2012
*/
...
#include "MotionSymbols2012.h"


using namespace naoth;
using namespace std;

void MotionSymbols2012::registerSymbols(xabsl::Engine& engine)
{
...
void MotionSymbols2012::execute()
{
...
void MotionSymbols2012::updateOutputSymbols()
{
...
MotionSymbols2012* MotionSymbols2012::theInstance = NULL;

// enum setter und getter
void MotionSymbols2012::setHeadMotionRequest(int value){ theInstance->headMotionRequest.id = (HeadMotionRequest::HeadMotionID)value; }
int MotionSymbols2012::getHeadMotionRequestId(){ return (int)(theInstance->headMotionRequest.id); }

int MotionSymbols2012::getHeadMotionStatus(){ return (int)(theInstance->motionStatus.headMotion); }


void MotionSymbols2012::setMotionRequestId(int value){ theInstance->motionRequest.id = (motion::MotionID)value; }
int MotionSymbols2012::getMotionRequestId(){ return (int)(theInstance->motionRequest.id); }

int MotionSymbols2012::getMotionStatusId(){ return (int)(theInstance->motionStatus.currentMotion); }
double MotionSymbols2012::getMotionStatusTime(){ return theInstance->frameInfo.getTimeSince(theInstance->motionStatus.time); }
bool MotionSymbols2012::getMotionStatusLeftMovable()
{ 
...
double MotionSymbols2012::relativeToRightFootY()
{
  const Kinematics::Link& l = theInstance->kinematicChain.theLinks[KinematicChain::RFoot];
  return relativeToY(theInstance->relativeToRightFootYx, theInstance->relativeToRightFootYy, l.R, l.p);
}

double MotionSymbols2012::relativeToRightFootA()
{
  const Kinematics::Link& l = theInstance->kinematicChain.theLinks[KinematicChain::RFoot];
  return relativeToA(theInstance->relativeToRightFootAa, l.R);
}

double MotionSymbols2012::relativeToX(double x, double y, const RotationMatrix& R, const Vector3<double>& p)
{
  return R[0].x * ( x - p.x ) + R[1].x *  ( y - p.y );
}

double MotionSymbols2012::relativeToY(double x, double y, const RotationMatrix& R, const Vector3<double>& p)
{
  return R[0].y * ( x - p.x ) + R[1].y * ( y - p.y );
}

double MotionSymbols2012::relativeToA(double a, const RotationMatrix& R)
{
  return Math::toDegrees(Math::normalize(Math::fromDegrees(a) - R.getZAngle()));
}

bool MotionSymbols2012::avoidObstacle()
{
...
void MotionSymbols2012::setAvoidObstacle(bool b)
{
...
string MotionSymbols2012::getStepControlFootName(StepControlFoot i)
{
  switch(i)
  {
