/**
* @file BallSymbols2012.cpp
*
* @author <a href="mailto:mellmann@informatik.hu-berlin.de">Heinrich Mellmann</a>
* Implementation of class BallSymbols2012
*/

#include "BallSymbols2012.h"

#include <Tools/Debug/DebugRequest.h>
#include <Tools/Debug/DebugBufferedOutput.h>

void BallSymbols2012::registerSymbols(xabsl::Engine& engine)
{
  ...
  DEBUG_REQUEST_REGISTER("XABSL:BallSymbols2012:ballLeftFoot", "draw the ball model in left foot's coordinates on field", false);
  DEBUG_REQUEST_REGISTER("XABSL:BallSymbols2012:ballRightFoot", "draw the ball model in right foot's coordinates on field", false);
...
void BallSymbols2012::execute()
{
  ...
  DEBUG_REQUEST("XABSL:BallSymbols2012:ballLeftFoot", 
    FIELD_DRAWING_CONTEXT;
    PEN("FF0000", 20);
    CIRCLE(ballLeftFoot.x, ballLeftFoot.y, 32.5);
    PLOT("XABSL:BallSymbols2012:ballLeftFoot:x", ballLeftFoot.x);
    PLOT("XABSL:BallSymbols2012:ballLeftFoot:y", ballLeftFoot.y);
  );

  DEBUG_REQUEST("XABSL:BallSymbols2012:ballRightFoot", 
    FIELD_DRAWING_CONTEXT;
    PEN("0000FF", 20);
    CIRCLE(ballRightFoot.x, ballRightFoot.y, 32.5);
    PLOT("XABSL:BallSymbols2012:ballRightFoot:x", ballRightFoot.x);
    PLOT("XABSL:BallSymbols2012:ballRightFoot:y", ballRightFoot.y);
  );
...
BallSymbols2012* BallSymbols2012::theInstance = NULL;


double BallSymbols2012::getBallSpeed()
{
  return theInstance->ballModel.speed.abs();
}

double BallSymbols2012::getBallDistance()
{
  return theInstance->ballModel.position.abs();
}//end getBallDistance

double BallSymbols2012::getFutureBallPosX()
{
...
double BallSymbols2012::getFutureBallPosY()
{
  int clipped = Math::clamp((int) theInstance->futureBallPosX_t, 0, BALLMODEL_MAX_FUTURE_SECONDS);
  return theInstance->ballModel.futurePosition[clipped].y;
}//end getBallDistance

double BallSymbols2012::getFutureBallDistance()
{
  int clipped = Math::clamp((int) theInstance->futureBallPosX_t, 0, BALLMODEL_MAX_FUTURE_SECONDS);
  return theInstance->ballModel.futurePosition[clipped].abs();
}//end getBallDistance

double BallSymbols2012::getBallAngle()
{
  return Math::toDegrees(theInstance->ballModel.position.angle());
}//end getBallAngle

double BallSymbols2012::getBallTimeSinceLastSeen()
{
...
double BallSymbols2012::getPosBehindBallFutureX()
{
  calculatePosBehindBallFuture();
  return theInstance->posBehindBall.translation.x;
}
double BallSymbols2012::getPosBehindBallFutureY()
{
  calculatePosBehindBallFuture();
  return theInstance->posBehindBall.translation.x;
}
double BallSymbols2012::getPosBehindBallFutureRotation()
{
  calculatePosBehindBallFuture();
  return Math::toDegrees(theInstance->posBehindBall.rotation);
}

double BallSymbols2012::getTeamBallGoalieTimeSinceLastUpdate()
{
  return theInstance->frameInfo.getTimeSince(theInstance->teamBallModel.goalieTime);
}//end getTeamBallGoalieTimeSinceLastUpdate

double BallSymbols2012::getTeamBallStrikerTimeSinceLastUpdate()
{
