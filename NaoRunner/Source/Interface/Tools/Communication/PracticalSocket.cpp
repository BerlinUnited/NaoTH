...
#define GET_NONBLOCK_ERROR WSAGetLastError()
#define NO_ERROR_WHEN_BLOCKING WSAEWOULDBLOCK
#else
#define GET_NONBLOCK_ERROR errno
#define NO_ERROR_WHEN_BLOCKING EAGAIN
#endif //WIN32
...
: isBlocking(true) /* throw(SocketException) */ {
  #ifdef WIN32
    if (!initialized) {
...
void Socket::configureBlocking(bool block)
{
  isBlocking = block;
}
...
void Socket::setBlockingOnce(bool blocking)
{
  #ifdef WIN32
  unsigned long nonblockingflag = blocking ? 0 : 1;
  ioctlsocket(sockDesc, FIONBIO, &nonblockingflag);
  #else
  long save_fd = fcntl(sockDesc, F_GETFL );
  if(blocking)
  {
    save_fd = save_fd & ~O_NONBLOCK;
  }
  else
  {
    save_fd = save_fd | O_NONBLOCK;
  }
  fcntl(sockDesc, F_SETFL, save_fd);
  #endif // WIN32
...
{
  setBlockingOnce(isBlocking);
  if (::send(sockDesc, (raw_type *) buffer, bufferLen, MSG_NOSIGNAL) < 0) {
...
int CommunicatingSocket::recv(void *buffer, int bufferLen) 
  ...
  if ((rtn = ::recv(sockDesc, (raw_type *) buffer, bufferLen, 0)) < 0) {
    int e = GET_NONBLOCK_ERROR;
    int t = NO_ERROR_WHEN_BLOCKING;
    if(isBlocking || e != t)
    {
