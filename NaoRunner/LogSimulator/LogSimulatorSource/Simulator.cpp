...
{
  comm = new CommunicationCollectionImpl();
...
void Simulator::init()
{
  Platform::getInstance().init(this, comm);
...
void Simulator::executeCurrentFrame()
{
  ...
  if(noFrameInfo)
  {
    unsigned int framePos = 1 + *currentFrame - *frames.begin();
    naothmessages::FrameInfo f;
    f.set_framenumber(*currentFrame);
    unsigned int frameTime = startFrameTime + abs((int)(framePos * CYCLE_TIME + lastFrameTime));
    f.set_time(frameTime);
